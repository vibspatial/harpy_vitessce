...
def visium_hd(
    ...
    visualize_as_rgb: bool = True,
    visualize_as_multiplex: bool | None = None,  # deprecated alias
    ...
    visualize_as_multiplex
        Deprecated alias for ``not visualize_as_rgb``. When provided, it
        overrides ``visualize_as_rgb``.
    ...
    if center is not None and len(center) != 2:
        raise ValueError("center must be a tuple of two floats: (x, y).")
    if visualize_as_multiplex is not None:
        logger.warning(
            "visualize_as_multiplex is deprecated; use visualize_as_rgb. "
            "Interpreting visualize_as_rgb = not visualize_as_multiplex."
        )
        visualize_as_rgb = not visualize_as_multiplex
